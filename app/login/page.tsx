				...
				{status === "loading" ? (
					...
					<>
						...
						<p className="serifBold">
							{siteConfig.login.yourAccountInfo}
						</p>
						<p>
						...
						<p>
							...
							{siteConfig.login.accountTypeLabels[
								session.user.access
							] || "Unknown"}
						</p>
						{session.user.access === 1 && session.user.teamID && (
							<p>
								{siteConfig.login.teamIdLabel}{" "}
								{session.user.teamID}
							</p>
						)}
						<br />
						{session.user.access >= 1 && (
							<p className="center">
								<Link href="/dashboard">
									<span className="button serifBold bBlue">
										Open My Hackathon Dashboard
									</span>
								</Link>
							</p>
						)}
						{session.user.access === 0 && (
							<>
								<p className="center">
									<a
										href={
											siteConfig.externalUrls.signUpForm
										}
										target="_blank"
